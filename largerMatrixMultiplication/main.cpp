...
/**
 * Multiply c certian rows of A and columns of B
 * @param c matrix A
 * @param d matrix B
...
*/
void matrixMultiply(double** c, double** d, double** result, int size, int start, int end){
    //multiply c and d
    //row of result
...
int main(void){
    ...
    //read matrix c
    double** c = read2d("c.mat", size, size);
    //read matrix d
    double** d = read2d("d.mat", size, size);
    //allocate matrix c
    ...
    for (int i = 0; i < NUM_THREADS; i++){
        ...
        ths[i] = std::thread(matrixMultiply, c, d, result, MATRIX_SIZE, start, end);
    }
    //engage all threads
    for (int i = 0; i < NUM_THREADS; i++){
        if(ths[i].joinable())
