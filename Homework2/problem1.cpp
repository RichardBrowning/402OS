...
void *supplier_process(void*){
    while(true){
        ...
        sequence = rand() % 3;
        if(sequence == 0){
            std::cout << "Agent supplies tob and paper" << std::endl;
            sem_post(&offer1);
        }else if(sequence == 1){
            std::cout << "Agent supplies paper and matches" << std::endl;
            sem_post(&offer2);
        }else if(sequence == 2){
            std::cout << "Agent supplies tob and glue" << std::endl;
            sem_post(&offer3);
        }
...
void* smoker1_process(void*){
    while(true){
        sem_wait(&offer3);
        //smoke
        std::cout << "Smoker 1 has tobacco, has paper and glue from the agent" << std::endl;
        sleep(1);
        sem_post(&finish);
    }
}
...
void* smoker2_process(void*){
    while(true){
        sem_wait(&offer2);
        //smoke
        std::cout << "Smoker 2 has paper, has tobacco and glue from the agent" << std::endl;
        sleep(1);
        sem_post(&finish);
    }
}
...
void* smoker3_process(void*){
    while(true){
        sem_wait(&offer1);
        //smoke
        std::cout << "Smoker 3 has glue, has tobacco and paper from the agent" << std::endl;
        sleep(1);
        sem_post(&finish);
    }
}
